...
class FeatureTransformation:
    ...
    def payload(self) -> Mapping[str, Any]:
        return {
            "schema": (
                "matrix.feature-transformation/1"
            ),
            "transformation_id": (
...
class SQLiteFeatureTransformationRegistry:
        ...
        *,
        ...
        outputs = tuple(
            sorted(
                {
                    _hex64(
                        "OUTPUT_FEATURE_FINGERPRINT",
                        value,
                    )
                    for value
                    in output_feature_definition_fingerprints
                }
            )
        )
        ...
        for item in inputs:
            ...
            fields = tuple(
                sorted(set(item.fields))
            )

            if len(fields) != len(item.fields):
                raise ValueError(
                    "DUPLICATE_TRANSFORMATION_INPUT_FIELD"
                )
        ...
        transformation_fingerprint = _sha(
            base
        )
        transformation_id = _sha(
            {
                "schema": (
                    "matrix.feature-transformation-id/1"
                ),
                "transformation_fingerprint": (
                    transformation_fingerprint
                ),
    ...
    def register(
        ...
        expected = self.build_transformation(
            ...
            artifact_sha256=(
                transformation.artifact_sha256
            ),
            output_feature_definition_fingerprints=(
                transformation
                .output_feature_definition_fingerprints
        ...
        if expected != transformation:
            raise ValueError(
                "TRANSFORMATION_DERIVATION_MISMATCH"
            )
        ...
        with self._connect() as connection:
            ...
            if existing is not None:
                ...
                raise ValueError(
                    "TRANSFORMATION_VERSION_MUTATION_VIOLATION"
                )
