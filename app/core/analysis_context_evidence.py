...
class AnalysisContextBundle:
    ...
    def payload(self) -> Mapping[str, Any]:
        return {
            "schema": "matrix.analysis-context-bundle/1",
            "sport": self.sport,
            "canonical_id": self.canonical_id,
            "history_profile_fingerprint": self.history_profile_fingerprint,
            "strength_of_schedule_fingerprint": (
                self.strength_of_schedule_fingerprint
            ),
            "load_context_fingerprint": self.load_context_fingerprint,
            "comparable_cohort_fingerprint": self.comparable_cohort_fingerprint,
...
def build_analysis_context_bundle(
    *,
    ...
    if (
        strength_of_schedule_profile.history_fingerprint
        != history_profile.history_fingerprint
    ):
        raise ValueError("HISTORY_LINEAGE_MISMATCH")
    ...
    base = {
        ...
        "history_profile_fingerprint": history_profile.profile_fingerprint,
        "strength_of_schedule_fingerprint": (
            strength_of_schedule_profile.profile_fingerprint
        ),
        "load_context_fingerprint": load_context.context_fingerprint,
        "comparable_cohort_fingerprint": comparable_fp,
        "point_in_time_enforced": True,
        "fatigue_score_computed": False,
        "recency_weighting_applied": False,
        "automatic_model_promotion": False,
...
class SQLiteAnalysisContextEvidenceStore:
    ...
    def record_bundle(
        ...
        if expected_fp != bundle.bundle_fingerprint:
            raise ValueError("ANALYSIS_CONTEXT_DERIVATION_MISMATCH")

        evidence_id = _sha(
            {
                "schema": "matrix.analysis-context-evidence-id/1",
        ...
        with self._connect() as connection:
            ...
            if existing is not None:
                ...
                if (
                    ...
                    return evidence_id
                raise ValueError("ANALYSIS_CONTEXT_MUTATION_VIOLATION")
            ...
            try:
                ...
                connection.execute("ROLLBACK")
                raise ValueError("ANALYSIS_CONTEXT_APPEND_ONLY_VIOLATION") from error

        return evidence_id

    def audit_integrity(self) -> AnalysisContextIntegrityReport:
        errors: list[str] = []
        with self._connect() as connection:
        ...
        for evidence_id, sport, canonical_id, bundle_fp, payload_json, stored_sha in rows:
            try:
                payload = json.loads(payload_json)
            except json.JSONDecodeError:
                errors.append(f"INVALID_JSON:{bundle_fp}")
                continue
            ...
            if actual_sha != stored_sha:
                errors.append(f"PAYLOAD_HASH_MISMATCH:{bundle_fp}")
            ...
            for key, expected in {
                ...
                if payload.get(key) != expected:
                    errors.append(f"{key.upper()}_MISMATCH:{bundle_fp}")
