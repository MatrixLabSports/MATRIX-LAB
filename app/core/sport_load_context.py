...
class SportLoadContext:
    ...
    def payload(self) -> Mapping[str, Any]:
        return {
            "schema": f"matrix.{self.sport}-load-context/1",
            "sport": self.sport,
            "canonical_id": self.canonical_id,
            "as_of": self.as_of.isoformat().replace("+00:00", "Z"),
            "event_at": self.event_at.isoformat().replace("+00:00", "Z"),
            "available_at": self.available_at.isoformat().replace("+00:00", "Z"),
            "rest_hours": self.rest_hours,
            "travel_km": self.travel_km,
            "timezone_shift_hours": self.timezone_shift_hours,
            "altitude_delta_m": self.altitude_delta_m,
            "matches_last_7d": self.matches_last_7d,
            "matches_last_14d": self.matches_last_14d,
            "cumulative_minutes_last_7d": self.cumulative_minutes_last_7d,
            "cumulative_minutes_last_14d": self.cumulative_minutes_last_14d,
...
def build_sport_load_context(
    *,
    ...
    if available_at > as_of:
        raise ValueError("CONTEXT_NOT_AVAILABLE_AS_OF")

    if event_at < as_of:
        raise ValueError("TARGET_EVENT_BEFORE_AS_OF")

    if previous_event_at is None:
        rest_hours = None
    else:
        previous_event_at = _aware("PREVIOUS_EVENT_AT", previous_event_at)
        if previous_event_at >= event_at:
            raise ValueError("INVALID_PREVIOUS_EVENT_AT")
        rest_hours = (event_at - previous_event_at).total_seconds() / 3600.0

    travel = None if travel_km is None else _finite(
        "TRAVEL_KM", travel_km, minimum=0
    )
    tz_shift = None if timezone_shift_hours is None else _finite(
        "TIMEZONE_SHIFT_HOURS", timezone_shift_hours
    )
    altitude = None if altitude_delta_m is None else _finite(
        "ALTITUDE_DELTA_M", altitude_delta_m
    )
    matches7 = _count("MATCHES_LAST_7D", matches_last_7d)
    matches14 = _count("MATCHES_LAST_14D", matches_last_14d)

    if matches7 > matches14:
        raise ValueError("MATCH_COUNT_WINDOW_INCONSISTENCY")

    minutes7 = None if cumulative_minutes_last_7d is None else _finite(
        "CUMULATIVE_MINUTES_LAST_7D",
        cumulative_minutes_last_7d,
        minimum=0,
    )
    minutes14 = None if cumulative_minutes_last_14d is None else _finite(
        "CUMULATIVE_MINUTES_LAST_14D",
        cumulative_minutes_last_14d,
        minimum=0,
    )
    if minutes7 is not None and minutes14 is not None and minutes7 > minutes14:
        raise ValueError("MINUTES_WINDOW_INCONSISTENCY")

    base = {
        "schema": f"matrix.{sport}-load-context/1",
        "sport": sport,
