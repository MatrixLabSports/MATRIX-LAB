...
from app.core.provider_request_contract import SQLiteProviderRequestContractRegistry
from app.core.secret_reference import SecretReference, resolve_secret_runtime
...
class JitSecretPinnedHttpsTransport(MatrixPinnedHttpsTransport):
    ...
    def __init__(
        ...
        *,
        ...
        resolver: Callable[[SecretReference], str] = resolve_secret_runtime,
    ) -> None:
    ...
    def get_pinned(
        ...
        **kwargs: Any,
        ...
        if expected_fp != self.secret_reference.reference_fingerprint:
            raise ValueError("REQUEST_SECRET_REFERENCE_MISMATCH")

        headers = dict(kwargs.pop("headers", {}))
        if any(
            str(key).lower() == self.auth_header_name.lower()
            for key in headers
        ):
            raise ValueError("CALLER_SECRET_HEADER_FORBIDDEN")

        secret_value = self.resolver(self.secret_reference)

        try:
            headers[self.auth_header_name] = secret_value
            ...
            headers.pop(self.auth_header_name, None)
            secret_value = None
...
class GovernedProviderRequestClient:
    ...
    def get(
        ...
        *,
        ...
        if self.secret_reference.provider_key != self.provider_key:
        ...
        decision = self.request_contract_registry.authorize(
            ...
            path=path,
            params=params,
            secret_reference_fingerprint=self.secret_reference.reference_fingerprint,
            now=self.clock(),
        )
        ...
        return self.governed_session.get(
            ...
            matrix_request_contract_id=decision.contract_id,
            matrix_request_contract_fingerprint=decision.authorization_fingerprint,
            matrix_request_path=decision.path,
