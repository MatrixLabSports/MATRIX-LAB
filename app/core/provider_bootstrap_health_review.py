...
class BootstrapHealthReview:
    ...
    def payload(self) -> Mapping[str, Any]:
        return {
            "schema": "matrix.bootstrap-health-review/1",
            ...
            "runtime_admission_evidence_fingerprint": (
                self.runtime_admission_evidence_fingerprint
            ),
...
class SQLiteBootstrapHealthReviewStore:
    ...
    def build_review(
        *,
        ...
        if decision not in {
            ...
            raise ValueError("INVALID_BOOTSTRAP_REVIEW_DECISION")
        ...
        review_fingerprint = _sha(base)
        review_id = _sha(
            {
                "schema": "matrix.bootstrap-health-review-id/1",
    ...
    def record(
        ...
        expected = self.build_review(
            ...
            bootstrap_run_id=review.bootstrap_run_id,
            reconciliation_fingerprint=review.reconciliation_fingerprint,
            runtime_admission_evidence_fingerprint=(
                review.runtime_admission_evidence_fingerprint
            ),
            reviewer_id=review.reviewer_id,
            decision=review.decision,
        )
        if expected != review:
            raise ValueError("BOOTSTRAP_HEALTH_REVIEW_DERIVATION_MISMATCH")

        payload_json = _canonical_json(review.payload())
        ...
        with self._connect() as connection:
            ...
            existing = connection.execute(
                """
                SELECT review_id, payload_sha256
            ...
            if existing is not None:
                ...
                if (
                    str(existing[0]) == review.review_id
                    and str(existing[1]) == payload_sha
                ):
                    return review
                raise ValueError("BOOTSTRAP_RUN_ALREADY_REVIEWED")
