...
def build_comparable_opponent_policy(
    *,
    ...
    if isinstance(max_absolute_quality_delta, bool) or not isinstance(
        max_absolute_quality_delta, (int, float)
    ):
    ...
    base = {
        ...
        "require_same_competition": require_same_competition,
...
def build_comparable_opponent_cohort(
    *,
    ...
    target = quality_ledger.resolve_as_of(
        sport=policy.sport,
        opponent_canonical_id=target_opponent_canonical_id,
        quality_key=policy.quality_key,
        as_of=as_of,
    )
    if target is None:
        raise ValueError("TARGET_OPPONENT_QUALITY_NOT_AVAILABLE_AS_OF")
    ...
    for event in events:
        if policy.require_same_competition:
            if target_competition_key is None:
                raise ValueError("TARGET_COMPETITION_REQUIRED")
            if event.competition_key != target_competition_key:
        ...
        historical_quality = quality_ledger.resolve_as_of(
            ...
            quality_key=policy.quality_key,
            as_of=min(as_of, event.source_available_at),
        )
        if historical_quality is None:
            missing += 1
            continue

        if abs(float(historical_quality["quality_value"]) - target_value) > (
    ...
    base = {
        "schema": "matrix.comparable-opponent-cohort/1",
        "sport": policy.sport,
        "target_opponent_canonical_id": target_opponent_canonical_id,
        "as_of": _iso(as_of),
        "target_quality_value": target_value,
        "target_quality_fingerprint": target["quality_fingerprint"],
        "selected_event_fingerprints": list(selected_fps),
