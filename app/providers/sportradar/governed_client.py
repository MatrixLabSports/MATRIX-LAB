...
def _build_controlled_sportradar_client(
    *,
    sport: str,
    config,
    authority,
    ...
    if not isinstance(config, SportradarConfig):
        raise ValueError("SPORTRADAR_CONFIG_REQUIRED")
    config._language_for_sport(sport)
    ...
    if secret_reference.provider_key != SPORTRADAR_PROVIDER_KEY:
        raise ValueError("SPORTRADAR_SECRET_REFERENCE_REQUIRED")
    if not isinstance(
        binding_store,
        SQLiteProviderNetworkBindingEvidenceStore,
    ):
...
def build_governed_sportradar_client(
    *,
    sport: str,
    config,
    authority,
    ...
    activation_readiness_certification: (
        ProviderActivationReadinessCertification | None
    ) = None,
    activation_rehearsal_certification: (
        ProviderActivationRehearsalCertification | None
    ) = None,
    ...
    require_real_provider_execution_authorized(network_certification)

    if activation_readiness_certification is None:
        raise ValueError("PROVIDER_ACTIVATION_READINESS_REQUIRED")
    if activation_rehearsal_certification is None:
        raise ValueError("PROVIDER_ACTIVATION_REHEARSAL_REQUIRED")
    ...
    return _build_controlled_sportradar_client(
        sport=sport,
        config=config,
        authority=authority,
        network_permit_store=network_permit_store,
