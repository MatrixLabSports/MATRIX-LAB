...
def test_comparable_cohort_uses_configured_quality_delta(
    tmp_path,
):
    ledger = SQLiteOpponentQualityLedger(
        tmp_path / "q.db"
    )
    quality(
        ledger,
        "tennis:player:target",
        1800,
        2,
        "a",
    )
    ...
    assert (
        cohort.selected_event_fingerprints
        == ("d" * 64,)
    )
    assert cohort.excluded_quality_delta == 1


def test_post_event_quality_is_not_retroactively_leaked(
    tmp_path,
):
    ledger = SQLiteOpponentQualityLedger(
        tmp_path / "q.db"
...
def test_same_competition_policy_is_explicit(
    tmp_path,
):
    ledger = SQLiteOpponentQualityLedger(
        tmp_path / "q.db"
