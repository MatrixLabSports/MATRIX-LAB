...
def test_strength_of_schedule_reports_coverage_not_fake_zero(
    tmp_path,
):
    ...
    assert (
        profile.payload()[
            "historical_quality_cutoff"
        ]
        == "event_at"
    )


def test_post_event_quality_is_not_used_for_sos(
    tmp_path,
):
    ledger = SQLiteOpponentQualityLedger(
        tmp_path / "q.db"
