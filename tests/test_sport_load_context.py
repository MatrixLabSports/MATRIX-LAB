...
def test_tennis_context_computes_rest_from_previous_event_end():
    context = build_tennis_load_context(
        **kwargs()
    )

    assert context.sport == "tennis"
    assert context.rest_hours == 70.0
    assert (
        context.payload()[
            "fatigue_score_computed"
        ]
        is False
    )
...
def test_football_context_remains_sport_separate():
    context = build_football_load_context(
        **kwargs()
    )
...
def test_context_requires_source_lineage():
    values = kwargs()
    values[
        "source_record_fingerprints"
    ] = ()
    ...
    with pytest.raises(
        ValueError,
        match="EMPTY_CONTEXT_SOURCE_RECORDS",
    ):
        build_tennis_load_context(**values)
